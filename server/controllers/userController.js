...
export const updateProfilePicture = asyncHandler(async (req, res, next) => {
  ...
  upload(req, res, async (err) => {
    if (err) {
      ...
      if (req.file) {
        //to resolve the bug of when i update my profile picture
        //the previous profile picture is not deleting to manage that
        //let create first a fileName and change findByIdAndUpdate to findById
        let fileName;
        ...
        if (fileName) {
          //if my filename is true i wanna use a file remover
          fileRemove(fileName);
        }
        //then i'm gonna update the user avatar
